...
export default function AgentLayout() {
  ...
  const { user } = useAuth();
  const { mobileNav, branding } = useSidebarRoleConfig('agent');
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-[280px] z-[60] lg:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <RoleSidebarNav routeRole="agent" isCollapsed={false} setIsSidebarCollapsed={setIsSidebarCollapsed} onLinkClick={() => setIsSidebarOpen(false)} />
      </aside>
      ...
      <aside className={`hidden lg:flex flex-col shrink-0 h-screen sticky top-0 bg-slate-900 overflow-hidden transition-all duration-300 ease-in-out z-50 shadow-[rgba(0,0,0,0.16)_0px_3px_6px,rgba(0,0,0,0.23)_0px_3px_6px] ${isSidebarCollapsed ? 'w-[80px]' : 'w-[260px]'}`}>
        <RoleSidebarNav routeRole="agent" isCollapsed={isSidebarCollapsed} setIsSidebarCollapsed={setIsSidebarCollapsed} onLinkClick={() => {}} />
      </aside>

      <div className="flex-1 flex flex-col min-w-0">
        {/* Premium Header */}
        <header className="h-20 flex items-center justify-between px-4 md:px-8 bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40 shadow-[rgba(0,0,0,0.16)_0px_3px_6px,rgba(0,0,0,0.23)_0px_3px_6px]">
          ...
          <div className="flex items-center gap-4">
            <button className="hidden sm:flex w-10 h-10 items-center justify-center rounded-xl bg-white border border-slate-100 text-slate-300 hover:text-primary transition-all relative">
              <Bell size={18} />
              <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-indigo-500 rounded-full border-2 border-white" />
            </button>

            <Link to={branding.profilePath || '/agent/profile'} className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-2xl hover:bg-slate-50 transition-all group border-none shadow-none bg-transparent">
              <div className="text-right hidden md:block">
                <p className="text-sm font-black text-slate-900 leading-none uppercase tracking-tight">{user?.name?.split(' ')[0] || 'Agent'}</p>
                <p className="text-[10px] font-bold text-slate-300 mt-1 uppercase tracking-widest leading-none">Field Representative</p>
              </div>
            ...
            {mobileNav.map(({ path, label, icon: Icon }) => {
              ...
              return (
                <Link key={path} to={pathStringToTo(path)}
                  ...
                  <div className={`p-2 rounded-xl transition-all ${active ? 'bg-primary/10 text-primary scale-110' : ''}`}>
                    <Icon size={20} strokeWidth={active ? 2.5 : 2} />
                  </div>
                  <span className={`text-[9px] mt-1 font-bold uppercase tracking-widest ${active ? 'text-primary' : 'text-slate-500'}`}>{label}</span>
                </Link>
              );
            })}
          </div>
