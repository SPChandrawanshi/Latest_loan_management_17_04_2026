...
export default function StaffLayout() {
  ...
  const { user } = useAuth();
  const { mobileNav, branding } = useSidebarRoleConfig('staff');
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-[280px] z-[60] lg:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <RoleSidebarNav routeRole="staff" isCollapsed={false} setIsSidebarCollapsed={setIsSidebarCollapsed} onLinkClick={() => setIsSidebarOpen(false)} />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 flex items-center justify-between px-4 md:px-8 bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40 shadow-[rgba(0,0,0,0.16)_0px_3px_6px,rgba(0,0,0,0.23)_0px_3px_6px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to={branding.profilePath || '/staff/profile'} className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-2xl hover:bg-slate-50 transition-all group border-none shadow-none bg-transparent">
              <div className="text-right hidden md:block">
                <p className="text-sm font-black text-slate-900 leading-none uppercase tracking-tight">{user?.name?.split(' ')[0] || 'Staff'}</p>
                <p className="text-[10px] font-bold text-slate-300 mt-1 uppercase tracking-widest leading-none">Corporate Officer</p>
              </div>
